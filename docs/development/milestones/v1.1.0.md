# Milestone: v1.1.0 - Knowledge Graph & Entity Extraction

Populate knowledge graphs during indexing and explore them via CLI.

## Theme

**"Connected Knowledge"** - Extract entities, build graphs, explore relationships.

## Features Planned

### Deferred from v1.0.0

| ID | Feature | Description | Status |
|----|---------|-------------|--------|
| F-124 | [Memory Optimisation](../features/planned/F-124-memory-optimisation.md) | Peak memory < 1GB, streaming embeddings | 📋 Planned |
| F-125 | [Startup Time](../features/planned/F-125-startup-time.md) | CLI startup < 500ms, lazy imports | 📋 Planned |
| F-126 | [Final Bug Fixes](../features/planned/F-126-final-bug-fixes.md) | Systematic audit and fixes | 📋 Planned |

### Entity Extraction & NER

| ID | Feature | Description | Status |
|----|---------|-------------|--------|
| F-128 | GLiNER NER | Zero-shot custom entity types | 📋 Planned |
| F-129 | EPUB Extraction | Ebook text extraction | 📋 Planned |
| F-130 | NER Pipeline Integration | Extract entities during indexing | 📋 Planned |

### Knowledge Graph CLI

| ID | Feature | Description | Status |
|----|---------|-------------|--------|
| F-131 | Knowledge Graph CLI | `ragd graph query`, explore, visualise | 📋 Planned |

> **Note:** Advanced CLI features (Comparison Framework, Batch Operations, Results Export, Timing & Metrics) moved to [v1.2.0](./v1.2.0.md) to reduce scope.

## Focus Areas

### Entity Extraction During Indexing (F-130)

The knowledge graph (v0.8.5) exists but remains empty because entity extraction isn't integrated into the indexing pipeline. F-130 closes this gap:

```bash
# Default: entities extracted during indexing
ragd add document.pdf
# → Entities extracted, graph populated

# Disable if needed
ragd add document.pdf --no-entities

# Custom entity types (requires GLiNER)
ragd add document.pdf --entity-types "TECHNOLOGY,CONCEPT"

# Rebuild entities for existing documents
ragd entities rebuild
```

### GLiNER Zero-Shot NER (F-128)

Extract custom entity types without training:

```bash
# Extract domain-specific entities
ragd add legal_brief.pdf --entity-types "STATUTE,CASE_NUMBER,COURT"
ragd add medical_paper.pdf --entity-types "DISEASE,DRUG,GENE"
```

### EPUB Support (F-129)

Expand document format support to ebooks:

```bash
ragd add book.epub
ragd list --format epub
```

### Knowledge Graph CLI (F-131)

```bash
# Query the graph
ragd graph query "entities related to machine learning"

# Explore connections
ragd graph explore --entity "neural networks" --depth 2

# Show statistics
ragd graph stats

# Export for visualisation
ragd graph export --format json > knowledge_graph.json

# Visualise (generates HTML/SVG)
ragd graph visualise --output graph.html
```

## Success Criteria

### Entity Extraction
- [ ] Entities extracted during `ragd add` by default
- [ ] Knowledge graph auto-populated
- [ ] GLiNER available for custom entity types
- [ ] EPUB files indexable

### Knowledge Graph CLI
- [ ] Graph exploration via CLI (`ragd graph query`, `ragd graph explore`)
- [ ] Graph statistics command (`ragd graph stats`)
- [ ] Graph visualisation export (`ragd graph visualise`)

### Performance (Deferred from v1.0.0)
- [ ] Peak memory < 1GB for large collections
- [ ] CLI startup < 500ms
- [ ] Systematic bug audit complete

## Dependencies

- v1.0.0 (Performance & Polish)
- F-022 (Knowledge Graph Foundation) - v0.8.5 ✅

---

## Related Documentation

### Milestones
- [v1.0.0 Milestone](./v1.0.0.md) - Previous release
- [v1.2.0 Milestone](./v1.2.0.md) - Next planned release

### Feature Specifications
- [F-128: GLiNER NER](../features/planned/F-128-gliner-ner.md)
- [F-129: EPUB Extraction](../features/planned/F-129-epub-extraction.md)
- [F-130: NER Pipeline Integration](../features/planned/F-130-ner-pipeline-integration.md)
- [F-022: Knowledge Graph](../features/completed/F-022-knowledge-graph.md)

### Research
- [State-of-the-Art NER Indexing](../research/state-of-the-art-ner-indexing.md)
- [State-of-the-Art NER](../research/state-of-the-art-ner.md)
- [State-of-the-Art Knowledge Graphs](../research/state-of-the-art-knowledge-graphs.md)

### Architecture Decisions
- [ADR-0034: GLiNER Zero-Shot NER](../decisions/adrs/0034-gliner-zero-shot-ner.md)
- [ADR-0031: Knowledge Graph Storage](../decisions/adrs/0031-knowledge-graph-storage.md)

---

**Status**: Planned
